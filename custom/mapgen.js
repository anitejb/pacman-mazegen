...
var getTiles = function() {
    ...
    for (y=1; y<subrows-1; y++) {
        if (getTile(subcols - 1,y) == ' ') {
            openingIndex = y;
            break;
        }
    }

    var allowedStartingOpeningIndices = [23]; // [8, 20, 23]
    if (!allowedStartingOpeningIndices.includes(openingIndex)) {
        genRandom();
        return getTiles();
    }
    ...
    if (openingIndex == 8) {
        // Open 8, 15, 22
        if (getTile(subcols - 2, 15) == '.' && getTile(subcols - 1, 15) == '|'
            && getTile(subcols - 2, 22) == '.' && getTile(subcols - 1, 22) == '|') {
            setTile(subcols-1, 15, ' ');
            setTile(subcols-1, 22, ' ');
        } else {
            genRandom();
            return getTiles();
        }
    } else if (openingIndex == 20) {
        // Open 10, 15, 20
        if (getTile(subcols - 2, 15) == '.' && getTile(subcols - 1, 15) == '|'
            && getTile(subcols - 2, 10) == '.' && getTile(subcols - 1, 10) == '|') {
            setTile(subcols-1, 15, ' ');
            setTile(subcols-1, 10, ' ');
        } else {
            genRandom();
            return getTiles();
        }
    } else if (openingIndex == 23) {
        // Open 7, 15, 23
        if (getTile(subcols - 2, 15) == '.' && getTile(subcols - 1, 15) == '|'
            && getTile(subcols - 2, 7) == '.' && getTile(subcols - 1, 7) == '|') {
            setTile(subcols-1, 15, ' ');
            setTile(subcols-1, 7, ' ');
        } else {
            genRandom();
...
var mapgen = function() {
    ...
    map.wallFillColor = "#DC3220"; // colorblind friendly red
    // map.wallStrokeColor = rgbString(hslToRgb(Math.random(), Math.random(), Math.random() * 0.4 + 0.6));
    map.wallStrokeColor = "#ffffff";
    map.pelletColor = "#ffb8ae";
    return map;
};

var mapgenGivenTiles = function(_tiles) {
    genRandom();
    var map = new Map(28,36,_tiles);
    map.name = "";
    // map.wallFillColor = randomColor();
    map.wallFillColor = "#005AB5"; // colorblind friendly blue
    // map.wallStrokeColor = rgbString(hslToRgb(Math.random(), Math.random(), Math.random() * 0.4 + 0.6));
    map.wallStrokeColor = "#ffffff";
    map.pelletColor = "#ffb8ae";
    return map;
