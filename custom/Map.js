...
Map.prototype.flipAcrossYAxis = function() {
    let flippedMap = [];
    for (let c = 0; c < this.tiles.length; c += this.numCols) {
        let a = this.tiles.slice(c, c+this.numCols);
        for (let i = this.numCols - 1; i >= 0; --i) {
            flippedMap.push(a[i]);
        }
    }
    return flippedMap.join("");
}

Map.prototype.flipAcrossXAxis = function() {
    let flippedMap = [];
    for (let c = this.tiles.length; c > 0; c -= this.numCols) {
        let a = this.tiles.slice(c-this.numCols, c);
